...
const buildSystemPrompt = (): string =>
{
    return [
        ...
        "      \"speaker\": string,         // OPTIONAL: this scene's default speaker",
        "      \"text\": [ string | { \"speaker\": string, \"text\": string } ], // ordered lines: narration and dialogue",
        ...
        "",
        "Rules:",
        "- A `text` entry is a plain string when the line is spoken by the scene's default speaker (its own `speaker`, or \"narrator\" if unset). Use the `{ speaker, text }` form only for a line whose speaker differs from that default.",
        "- Keep narration and dialogue in separate lines, never mix a narration clause and a quoted line in the same `text` entry.",
