<script lang="ts">
	import { storiesMeta } from '$lib/data/index';
...
|____/ |_| \___/|_| \_\___|_____|____/
	...
	<div class="border border-terminal-dim rounded px-2 py-1 mb-4">
		{#each storiesMeta as story, i}
			<button
				class="w-full text-left px-3 py-3 rounded transition-all duration-100 block
					{i === selectedIndex
						? 'bg-terminal-green bg-opacity-15 border-l-2 border-terminal-green'
						: 'border-l-2 border-transparent hover:bg-white hover:bg-opacity-5'}"
			...
			{#if i < storiesMeta.length - 1}
				<div class="border-t border-terminal-dim border-opacity-20 mx-3"></div>
			{/if}
		{/each}
	</div>
